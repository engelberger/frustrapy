...
def xadens(pdb: "Pdb", ratio: float = 5) -> None:
    ...
    ca_points = pd.DataFrame(ca_xyz, columns=['x', 'y', 'z'])

    # Convert vps columns to float
    vps['col3'] = vps['col3'].astype(float)
    vps['col4'] = vps['col4'].astype(float)
    vps['col5'] = vps['col5'].astype(float)
    vps['col6'] = vps['col6'].astype(float)

    distances = np.sqrt(
        (ca_points['x'].values.reshape(-1, 1) - vps['col3'].values) ** 2 +
        (ca_points['y'].values.reshape(-1, 1) - vps['col4'].values) ** 2 +
        (ca_points['z'].values.reshape(-1, 1) - vps['col5'].values) ** 2
    )

    for i in range(len(ca_points)):
        total_density = np.sum(distances[i] < ratio)
        highly_frustrated = np.sum((distances[i] < ratio) & (vps["col6"] <= -1))
        neutral_frustrated = np.sum((distances[i] < ratio) & (vps["col6"] > -1) & (vps["col6"] < 0.78))
        minimally_frustrated = np.sum((distances[i] < ratio) & (vps["col6"] >= 0.78))
